// Cloudflare Pages Function - AoRE Admin (GUI Restored + AES-GCM Encrypted)
...
const AORE_BUILD = "12-6-13-PRO-SECURE"
  ...
  <section class="aore-hero">
    <div class="aore-wrap">
      <div class="aore-hero-inner">
        ...
        <p class="aore-lead">
          Your registration was received securely (AES-GCM Encrypted).<br>
  ...
  <section class="aore-section">
    <div class="aore-wrap">
      <div class="aore-card">
        ...
        <div class="aore-card-sub">Ticket created - intake stored securely.</div>
      </div>
    </div>
  </section>
  <footer><p>© AoRE Labs 2006-2026</p></footer>
</body>
</html>`
}
  ...
  <section class="aore-hero">
    <div class="aore-wrap">
      <div class="aore-hero-inner">
        ...
        <p class="aore-lead">The request could not be processed.</p>
      </div>
    </div>
  </section>
  <section class="aore-section">
...
// GUI ORIGINAL PARA PANEL DE USUARIOS
...
<head>
  ...
  <style>
    ...
    .enc-badge{background:#0e1b38;color:#b98eff;padding:2px 6px;border-radius:4px;font-size:10px;margin-left:6px;border:1px solid #2d3f74}
  </style>
</head>
<body>
  <header>
...
<td class="key">${htmlEscape(key)}</td>
  ...
  if (path === "/api/register" && request.method === "POST") {
    if (!env.AORE_USERS) {
      return new Response(registerFailPage("Server not configured: missing AORE_USERS binding."), { status: 500, headers: { "content-type": "text/html; charset=utf-8" } })
    }

    // REQUIRE ENCRYPTION KEY
    if (!env.KV_MASTER_KEY) {
      return new Response(registerFailPage("Encryption System Error (Missing Master Key)."), { status: 500, headers: { "content-type": "text/html; charset=utf-8" } })
    }
    ...
    if (!name || !email || !password || !confirmPassword) {
      return new Response(registerFailPage("Missing required fields."), { status: 400, headers: { "content-type": "text/html; charset=utf-8" } })
    }

    if (password !== confirmPassword) {
      return new Response(registerFailPage("Passwords do not match."), { status: 400, headers: { "content-type": "text/html; charset=utf-8" } })
    }

    if (password.length < 8) {
      return new Response(registerFailPage("Password too short. Minimum is 8 characters."), { status: 400, headers: { "content-type": "text/html; charset=utf-8" } })
    }

    if (message.length > 800) {
      return new Response(registerFailPage("Message too long. Limit is 800 characters."), { status: 400, headers: { "content-type": "text/html; charset=utf-8" } })
    }

    const vt = await verifyTurnstile(env, request, tokenTs)
    if (!vt.ok) {
      return new Response(registerFailPage("Human check failed."), { status: 403, headers: { "content-type": "text/html; charset=utf-8" } })
    }
    ...
    try {
      ...
      await env.AORE_USERS.put(key, dataToStore)
    } catch (e) {
